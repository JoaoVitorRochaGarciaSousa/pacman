...
public class Game extends JFrame implements KeyListener {
	...
	private void run() {
		while (player.getLife() > 0) {
			// Quando a vida for maior que zero e se a função de botão pressionado estiver em true, então o player ira se mover
			if(paracima == true){
				player.mover();
			}   
			...
			if(!player.isInvencivel() && player.colisao(ghost1) || player.colisao(ghost2) || player.colisao(ghost3) || player.colisao(ghost4) || player.colisao(bomb)) {        //coloque aqui os métodos de movimentação e colisão 
				System.out.println("Voce recebeu dano e perdeu uma vida"+System.currentTimeMillis());
				Dano();
